// HotspotPro — two processes, one shared brain.
//
//   com.apple.springboard  the collector: a timer that folds interface counters
//                          into the persisted total and posts the soft-limit
//                          warnings. Reads sysctls and files, nothing else.
//   com.apple.Preferences  the UI: extra groups appended to the stock Personal
//                          Hotspot pane, reading the state the collector wrote.
//
// Everything that runs inside SpringBoard is wrapped in @try/@catch. An
// uncaught exception on SpringBoard's main thread is a Safe Mode boot loop, and
// no usage counter is worth that.
